...
class LongMethod(config: Config = Config.empty) : Rule(config) {
    ...
    @Configuration("number of lines in a method to trigger the rule for composable")
    private val composeThreshold: Int by config(defaultValue = 100)
    ...
    override fun postVisit(root: KtFile) {
        ...
        for ((function, lines) in functionToLines) {
            val isComposable = function.annotationEntries.any {
                it.name == "androidx.compose.runtime.Composable" || it.shortName?.toString() == "Composable"
            }
            val threshold = if (isComposable) composeThreshold else this.threshold
            if (lines >= threshold) {
                report(
